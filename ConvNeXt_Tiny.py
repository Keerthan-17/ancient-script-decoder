...
# ----------------- Load ConvNeXt Tiny -----------------
model = models.convnext_tiny(weights=None)  # Do not load ImageNet weights if custom
num_classes = len(class_names)
model.classifier[2] = torch.nn.Linear(model.classifier[2].in_features, num_classes)

# Load your trained weights
model.load_state_dict(torch.load("D:\Desktop\Ensemble\convnext_tiny_best_model1.pth", map_location=DEVICE))
model.to(DEVICE)
model.eval()
...
iface = gr.Interface(
    ...
    title="ConvNeXt Tiny Classifier",
